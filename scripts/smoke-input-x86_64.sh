...
log="build/x86_64/smoke-input.log"
...
if [[ ${rc} -ne 0 && ${rc} -ne 124 ]]; then
  echo "smoke-input-x86_64: qemu failed rc=${rc}" >&2
  tail -n 180 "${log}" >&2 || true
  exit 1
fi

if ! tr -d '\r' <"${log}" | grep -Fq "${boot_marker}"; then
  echo "smoke-input-x86_64: FAIL (missing boot marker)" >&2
  tail -n 180 "${log}" >&2 || true
  exit 1
fi
if ! tr -d '\r' <"${log}" | grep -Fq "${input_marker}"; then
  echo "smoke-input-x86_64: FAIL (missing input marker)" >&2
  tail -n 180 "${log}" >&2 || true
  exit 1
fi
if ! tr -d '\r' <"${log}" | grep -Fq "${user_marker}"; then
  echo "smoke-input-x86_64: FAIL (missing user marker)" >&2
  tail -n 180 "${log}" >&2 || true
  exit 1
fi
if tr -d '\r' <"${log}" | grep -Eq "interrupt: unhandled|exception: "; then
  echo "smoke-input-x86_64: FAIL (unexpected interrupt/exception)" >&2
  tail -n 200 "${log}" >&2 || true
  exit 1
fi

echo "smoke-input-x86_64: PASS"
