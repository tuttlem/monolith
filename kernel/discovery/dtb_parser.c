...
void hw_discovery_parse_dtb(const boot_info_t *boot_info, hw_desc_t *desc) {
  ...
  BOOT_U32 depth = 0;
  BOOT_U64 last_reg_base = 0;
  BOOT_U64 last_reg_size = 0;
  BOOT_U64 last_irq = 0;
  BOOT_U64 node_is_cpu = 0;
  BOOT_U64 node_is_timer = 0;
  BOOT_U64 node_is_uart = 0;
  BOOT_U64 node_is_irqc = 0;
  BOOT_U64 node_irqc_type = HW_IRQ_CONTROLLER_UNKNOWN;
  BOOT_U64 cpu_id_from_reg = 0;
  ...
  while (p + 4 <= struct_end) {
    ...
    if (token == FDT_BEGIN_NODE) {
      ...
      node_is_cpu = (str_eq(name, "cpu") || str_starts_with(name, "cpu@")) ? 1ULL : 0ULL;
      node_is_timer = (str_eq(name, "timer") || str_starts_with(name, "timer@")) ? 1ULL : 0ULL;
      node_is_uart = (str_starts_with(name, "uart") || str_starts_with(name, "serial")) ? 1ULL : 0ULL;
      node_is_irqc = 0ULL;
      node_irqc_type = HW_IRQ_CONTROLLER_UNKNOWN;
      if (str_starts_with(name, "plic")) {
        node_irqc_type = HW_IRQ_CONTROLLER_RISCV_PLIC;
      } else if (str_starts_with(name, "gic")) {
        node_irqc_type = HW_IRQ_CONTROLLER_ARM_GIC;
      }
      last_reg_base = 0;
      last_reg_size = 0;
      last_irq = 0;
    ...
    if (token == FDT_END_NODE) {
      if (node_is_cpu != 0ULL) {
        add_cpu(desc, cpu_id_from_reg);
      }
      if (node_is_timer != 0ULL) {
        add_timer(desc, last_irq);
      }
      if (node_is_irqc != 0ULL) {
        add_irqc(desc, node_irqc_type == HW_IRQ_CONTROLLER_UNKNOWN ? HW_IRQ_CONTROLLER_UNKNOWN : node_irqc_type,
                 last_reg_base, last_reg_size);
      }
      if (node_is_uart != 0ULL && last_reg_base != 0ULL) {
        add_uart(desc, last_reg_base, last_irq);
      }
      if (depth == 0U) {
        break;
      }
    ...
    if (token == FDT_PROP) {
      ...
      if (str_eq(prop_name, "reg")) {
        last_reg_base = read_reg64(p, len, &last_reg_size);
        if (node_is_cpu != 0ULL) {
          cpu_id_from_reg = last_reg_base;
        }
      } else if (str_eq(prop_name, "interrupts")) {
        if (len >= 4U) {
          last_irq = (BOOT_U64)be32_at(p);
        }
