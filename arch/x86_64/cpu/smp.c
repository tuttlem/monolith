...
static void EFIAPI x86_smp_ap_entry(VOID *arg) {
  x86_smp_context_t *ctx = (x86_smp_context_t *)arg;
  UINTN cpu_id = 0;

  if (ctx == (x86_smp_context_t *)0 || ctx->mp == (EFI_MP_SERVICES_PROTOCOL *)0 || ctx->mp->WhoAmI == (EFI_MP_WHOAMI)0) {
    return;
  }
...
status_t arch_smp_bootstrap(const boot_info_t *boot_info, BOOT_U64 *out_possible_cpus, BOOT_U64 *out_started_cpus) {
  ...
  *out_possible_cpus = (BOOT_U64)total;
  ...
  ctx.mp = mp;
  ctx.started = 0;
  st = mp->StartupAllAPs(mp, x86_smp_ap_entry, 0, (EFI_EVENT)0, 2000000U, &ctx, (UINTN **)0);
