...
status_t arch_memory_init(boot_info_t *boot_info) {
  ...
  sparc64_early_paging_result_t paging_result;
  ...
  if (!sparc64_early_paging_takeover(&paging_result)) {
    sparc_ext->mem_init_status = BOOT_MEM_INIT_STATUS_FAILED;
    return STATUS_INTERNAL;
  }
  ...
  sparc_ext->mem_init_status = BOOT_MEM_INIT_STATUS_DONE;
  sparc_ext->mem_old_root = paging_result.old_root;
  sparc_ext->mem_new_root = paging_result.new_root;
  sparc_ext->mem_mapped_bytes = usable_bytes(boot_info);
  return STATUS_OK;
}
